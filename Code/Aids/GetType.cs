...
namespace Abc.Aids
{
    ...
    {
        public static IEnumerable<PropertyInfo> Properties<TClass>(BindingFlags f) => typeof(TClass).GetProperties(f);
        public static IEnumerable<string> PropertyNames<TClass>(BindingFlags f) => Properties<TClass>(f).Select(p => p.Name);
